 ...
 * @LastEditors  : peng.li
 * @LastEditTime : 2023-11-12 14:50:47
                ...
                await new Promise((resolve2, reject2) => {
                    let cb = (coding) => {
                        fs.readFile(filePath, coding, async (err, data) => {
                            ...
                            if (data.indexOf('_LC(0') === -1) return resolve2(false);

                            // 找出代码中所有的调用_LC的函数
                            const matchArr = data.match(/_LC\(0[^)]+\)/g);
                            // console.log(matchArr);
                            if (matchArr) {
                                // "_LC(0, "内容")";
                                for (let needChangeContext of matchArr) {
                                    // 取出编号
                                    let matchIndexResult = needChangeContext.match(/\d+/g);
                                    if (!matchIndexResult) continue;

                                    let nextId;

                                    // 筛选出带双引号的文本
                                    const regex = /"([^"]*)"/g;
                                    const matches = needChangeContext.match(regex);
                                    if (typeof matches !== "Array" || typeof matches !== "array") {
                                        console.log("")
                                    }
                                    ...
                                    // 如果代码中有"_LC(0" 这种的， 直接替换
                                    data = data.replace("_LC(0", "_LC(" + nextId);
                                }
                            }
                            resolve2(true);
                                ...
                                if (hasChineseCharacters(content)) {
                                    ...
                                    if (result) {
                                        ...
                                        if (!isContentChange) {
                                            // 记录内容，更改内容
                                            let nextId = genLocalizeId();
                                            uiKey = "EXCEL_TO_LANGUAGE_" + nextId;
...
tryChangeUI(pathUI, collectUILanguage).then(() => {
    ...
    wirteToLanguage("UI_TO_LANGUAGE_", collectUILanguage);
    ...
    tryChangeExcel(pathAllExcel, collectExcelLanguage).then(() => {
        ...
        // 写入Language_多语言表.xlsx
        wirteToLanguage("EXCEL_TO_LANGUAGE_", collectExcelLanguage);
        ...
        tryChangeScript(pathZ, collectScroptResult).then(() => {
            ...
            wirteToLanguage("SCRIPT_TO_LANGUAGE_", collectScroptResult, true);
        });
    });
});